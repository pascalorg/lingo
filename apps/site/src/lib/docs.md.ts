...
export const docsMarkdown = [
  ...
  '',
  'Inject it into a field with `lingoInput({ complete, onComplete })` or `useLingoInput({ complete })`; the engine is never bundled into `@pascal-app/lingo/dom` or `/react`, and no popup UI ships. Pass `date` to opt into date completions without pulling `@pascal-app/lingo/date` into `/complete`. Pass `calc` with `{ trigger: "=" }` so `=2+3 kg` completes as an evaluated quantity without bundling `@pascal-app/lingo/calc`.',
  '',
  '### Find values in text',
  '',
...
useForm({ resolver: standardSchemaResolver(shipment) })`,
  ...
  '',
  'Inject `calc` into `completions({ calc })` and `quantityField({ calc })` so `=2+3 kg`, `12 * 0.75 kg`, and `10 kg / 2` evaluate without bundling the calculator into those entries. Completions only fire on a leading `=`, so `5-10 kg` is not stolen. Glued `5/10 kg` stays a fraction. `rangeField` stays range-first.',
  '',
  '## Locales',
  '',
...
lingo.parse('72 in to cm') // locale: 'en'`,
  ...
  '',
  'Issues are `{ code, severity, message, span, suggestions?, data? }`; parse-path spans are `{ start, end }` half-open offsets into the original input. Issue codes documented by the package: `EMPTY`, `NO_VALUE`, `UNKNOWN_UNIT`, `KIND_MISMATCH`, `RANGE_KIND_MISMATCH`, `CONVERSION_KIND_MISMATCH`, `RATE_REQUIRED`, `TRAILING_INPUT`, `SINGLE_VALUE_EXPECTED`, `APPROX_NOT_ALLOWED`, `UNIT_REQUIRED`, `CONVERSION_NOT_ALLOWED`, `NUMBER_FORMAT`, `NONFINITE`, `LOCALE_NOT_LOADED`, `RANGE_MIN`, `RANGE_MAX`, `RANGE_OPEN_BOUND_NOT_ALLOWED`, `REQUIRED`, `UNSUPPORTED_DATE`, `NOW_REQUIRED`, `TYPO_CORRECTED`, `AMBIGUOUS_NUMBER`, `AMBIGUOUS_UNIT`, `AMBIGUOUS_DATE`, `RANGE_REVERSED`, `COMPOUND_OVERFLOW`, `CIVIL_AVERAGE`, `UNIT_ASSUMED`, `WEEKDAY_ASSUMED_NEXT`, `SLANG_UNIT`, `TZ_IGNORED`, `AMBIGUOUS_TIMEZONE`, `AFFINE_DELTA_ASSUMED`, `EXPRESSION_KIND_MISMATCH`, `SCALAR_EXPECTED`, `DIVISION_BY_ZERO`, `SCALE_ASSUMED`.',
  '',
  '### Data schemas',
  '',
